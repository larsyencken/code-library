...
def iuniqueTuples(inputList, n=2):
    ...
    import itertools
    return itertools.ifilter(filterFn, icombinations(n*[inputList]))
...
def stddev(sequence):
    ...
    return math.sqrt((sumSquared - total**2/n) / (n-1))

#----------------------------------------------------------------------------#

def basicStats(sequence):
    total = 0.0
    sumSquared = 0.0
    n = 0
    for item in sequence:
    ...
    stddevVal = math.sqrt((sumSquared - total**2/n) / (n-1))
    meanVal = total / n
...
#FreqDist.__doc__ = \
#""" This class represents a simple frequency distribution. It counts the
#    number of times an each distinct item has occurred, and gives the MLE
#    probability estimate based on those counts.
#"""
