...
class BasicStatsTest(unittest.TestCase):
    ...
    def testBinsByRange(self):
        ...
        self.assertEqual(
                list(stats.binsByRange(data, 3, lambda x: x)),
                zip(expectedLabels, expectedBins)
            )
    ...
    def testBinsByInc(self):
        ...
        self.assertEqual(
                list(stats.binsByIncrement(data, 3.0, lambda x: x)),
                zip(expectedLabels, expectedBins)
            )
    ...
    def testSegmentCombinations(self):
        ...
        self.assertEqual(
                set(stats.segmentCombinations(['d', 'o', 'g'])),
                result
            )
        self.assertEqual(
                set(stats.isegmentCombinations(['d', 'o', 'g'])),
                result
            )
...
class KappaTest(unittest.TestCase):
    ...
    def testHighKappa(self):
        ...
        """
        kappaVal = stats.kappa(self.dataA, self.dataA)
    ...
    def testNoAgreement(self):
        ...
        """
        kappaVal = stats.kappa(self.dataA, self.dataB)
        self.assertAlmostEqual(kappaVal, -0.23076923076923078)
        return

    def testZeroKappa(self):
        kappaVal = stats.kappa(self.dataA, [0,0,6,6])
...
class CombinationTest(unittest.TestCase):
    ...
    def testCombinations(self):
        self.assertEqual(stats.combinations(self.dataA, self.dataA),
                [(1,1),(2,1),(3,1),(1,2),(2,2),(3,2),(1,3),(2,3),(3,3)])
        return

    def testICombinations(self):
        self.assertEqual(list(stats.icombinations(self.dataA, self.dataA)),
                [(1,1),(2,1),(3,1),(1,2),(2,2),(3,2),(1,3),(2,3),(3,3)])

        return

    def testUniqueTuples(self):
        self.assertEqual(stats.uniqueTuples([1,2]), [(1,2)])
        self.assertEqual(stats.uniqueTuples([1,2,3]), [(1,2), (1,3), (2,3)])

        return

    def testInclusionCombination(self):
        self.assertEqual(
                set(map(tuple, stats.inclusionCombinations([1,2]))),
                set(map(tuple, [[], [1], [2], [1,2]])),
            )
        self.assertEqual(stats.inclusionCombinations([]), [[]])
        return

    def testIUniqueTuples(self):
        self.assertEqual(list(stats.iuniqueTuples([1,2])), [(1,2)])
        self.assertEqual(list(stats.iuniqueTuples([1,2,3])), [(1,2), (1,3), (2,3)])
